...
interface HeaderProps {
  ...
  onExport: (format: 'png' | 'jpeg' | 'svg' | 'pdf' | 'deck-pdf' | 'nwsdraw') => void;
  onImportFile: (file: File) => void;
  onImportImages: (files: File[], asSlides: boolean) => void;
  onImportPdf: (file: File, asSlides: boolean) => void;
}
...
export const Header: React.FC<HeaderProps> = ({
  ...
  const imgCanvasRef = useRef<HTMLInputElement>(null);
  const imgSlidesRef = useRef<HTMLInputElement>(null);
  const pdfCanvasRef = useRef<HTMLInputElement>(null);
  const pdfSlidesRef = useRef<HTMLInputElement>(null);
  ...
  const handleImagePick = (e: React.ChangeEvent<HTMLInputElement>, asSlides: boolean) => {
    const files = [...(e.target.files || [])];
    if (files.length > 0) onImportImages(files, asSlides);
    e.target.value = '';
  };

  const handlePdfPick = (e: React.ChangeEvent<HTMLInputElement>, asSlides: boolean) => {
    const file = e.target.files?.[0];
    if (file) onImportPdf(file, asSlides);
    e.target.value = '';
  };
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="relative">
          <button
            ...
            className="p-2 sm:px-3 sm:py-1.5 text-xs font-medium text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition flex items-center gap-1.5"
            title="Import images, PDFs & project files"
          >
          ...
          {isImportOpen && (
            <>
              ...
              <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl p-1.5 z-50 animate-in fade-in zoom-in-95 duration-100">
                <div className="px-2 py-1 text-[10px] font-bold uppercase tracking-wider text-slate-400">
                  Onto the canvas
                </div>
                <button
                  onClick={() => {
                    imgCanvasRef.current?.click();
                ...
                >
                  <span>Open .nwsdraw file</span>
                  <FileCode className="w-3.5 h-3.5" />
                </button>

                <p className="px-2.5 pt-1.5 pb-1 text-[10px] leading-relaxed text-slate-400">
                  Tip: drag files onto the canvas or paste an image with Cmd+V.
                </p>
              </div>
            </>
          )}
              ...
              <div className="absolute right-0 mt-2 w-52 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl p-1.5 z-50 animate-in fade-in zoom-in-95 duration-100">
                ...
                {project.mode === 'presentation' && (
                  <button
                    ...
                    className="w-full text-left px-2.5 py-1.5 rounded-lg text-xs font-medium text-amber-600 dark:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-950/40 transition"
                  >
                    Export deck as PDF (all slides)
                  </button>
                )}
        ...
        <input
          ...
          onChange={(e) => handlePdfPick(e, false)}
          className="hidden"
        />
        <input
          type="file"
          ref={pdfSlidesRef}
          accept="application/pdf,.pdf"
          onChange={(e) => handlePdfPick(e, true)}
          className="hidden"
        />
